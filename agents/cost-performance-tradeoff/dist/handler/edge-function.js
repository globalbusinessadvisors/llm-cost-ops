...
export async function handler(req, res) {
    ...
    try {
        ...
        if (!parseResult.success) {
            ...
            await telemetry.emitError(AGENT_ID, new Error('Input validation failed'), { errors: parseResult.error.errors }, traceId);
        ...
        // Emit telemetry
        await telemetry.emitTradeoffAnalysis(AGENT_ID, output.analysis_id, output.summary.total_records_analyzed, executionTimeMs, traceId);
        ...
        const executionTimeMs = Date.now() - startTime;
        await telemetry.emitError(AGENT_ID, error, { execution_time_ms: executionTimeMs }, traceId);
        await telemetry.flush();
        telemetry.stop();
        const errorResponse = {
            success: false,
