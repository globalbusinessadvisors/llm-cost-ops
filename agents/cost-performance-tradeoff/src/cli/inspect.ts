...
import type { InspectCommandOptions, DecisionEvent } from '../contracts/index.js';
      ...
      try {
        ...
        if (options.model) {
          filtered = filtered.filter(e =>
            JSON.stringify(e.outputs).includes(options.model!)
          );
        }
        if (options.provider) {
          filtered = filtered.filter(e =>
            JSON.stringify(e.outputs).includes(options.provider!)
          );
        }
...
function formatEventsTable(events: DecisionEvent[]): string {
  const lines: string[] = [];
  ...
  for (const event of events) {
    const outputs = event.outputs as {
