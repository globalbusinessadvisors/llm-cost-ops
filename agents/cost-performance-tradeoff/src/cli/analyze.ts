...
import type { AnalyzeCommandOptions, TradeoffAnalysisInput } from '../contracts/index.js';
  ...
  cmd
    ...
    .action(async (options: AnalyzeCommandOptions & {
      ...
      try {
        ...
        const analysisInput: TradeoffAnalysisInput = {
          records: input.records,
          analysis_scope: options.scope as TradeoffAnalysisInput['analysis_scope'],
          weights,
          constraints: {
        ...
        if (options.summary) {
          ...
          console.log(formatter.format(output, options.format as 'json' | 'table'));
        }
