...
import type {
  PerformanceRecord,
  TradeoffAnalysisInput,
  TradeoffAnalysisOutput,
  TradeoffResult,
  TradeoffScore,
...
export class TradeoffAnalyzer {
   ...
   */
  private averageCostMetrics(records: PerformanceRecord[]): CostMetrics {
    const n = records.length;

    const totalCost = records.reduce(
      (sum, r) => sum + r.cost.total_cost_usd,
      0
    );
    const totalTokens = records.reduce(
      (sum, r) => sum + r.cost.token_count,
      0
    );
    const avgCostPerRequest = records.reduce(
      (sum, r) => sum + r.cost.cost_per_request_usd,
      0
    ) / n;
    const avgCostPer1kTokens = records.reduce(
      (sum, r) => sum + r.cost.cost_per_1k_tokens_usd,
      0
    ) / n;
   ...
   */
  private averageLatencyMetrics(records: PerformanceRecord[]): LatencyMetrics {
    const n = records.length;
   ...
   */
  private averageQualityMetrics(records: PerformanceRecord[]): QualityMetrics | undefined {
  ...
  private calculateTradeoffScores(
    ...
    return aggregated.map(agg => {
      ...
      const tradeoffScore: TradeoffScore = {
   ...
   */
  private computeParetoFrontier(results: TradeoffResult[]): ParetoPoint[] {
   ...
   */
  private detectDiminishingReturns(results: TradeoffResult[]): DiminishingReturnsAnalysis {
    // Sort by cost
    ...
    const marginalGains: { cost: number; gain: number }[] = [];
    ...
    let thresholdCost: number | undefined;
    let lastPositiveMarginalGain: number | undefined;
  ...
  private generateRecommendations(
    ...
    const recommendations: TradeoffRecommendation[] = [];
  ...
  private findBestByMetric(
    results: TradeoffResult[],
    metric: 'cost' | 'latency' | 'quality' | 'overall'
  ): TradeoffResult | undefined {
    if (results.length === 0) return undefined;

    switch (metric) {
      case 'cost':
        return results.reduce((best, r) =>
          r.avg_cost.cost_per_request_usd < best.avg_cost.cost_per_request_usd ? r : best
        );
      case 'latency':
        return results.reduce((best, r) =>
          r.avg_latency.p95_ms < best.avg_latency.p95_ms ? r : best
        );
      case 'quality':
        return results
          .filter(r => r.avg_quality !== undefined)
          .reduce((best, r) =>
            (r.avg_quality?.composite_score ?? 0) > (best.avg_quality?.composite_score ?? 0) ? r : best
          , results[0]);
      case 'overall':
        return results.reduce((best, r) =>
          r.tradeoff_score.overall_score > best.tradeoff_score.overall_score ? r : best
        );
    }
  }
  ...
  private createRecommendation(
    type: TradeoffRecommendation['recommendation_type'],
    best: TradeoffResult,
    allResults: TradeoffResult[],
    rationale: string
  ): TradeoffRecommendation {
